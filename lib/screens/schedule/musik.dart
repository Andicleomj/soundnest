...
class _MusikScheduleFormState extends State<MusikScheduleForm> {
  List<Map<String, String>> selectedMusicList = [];
  TimeOfDay? startTime;
  TimeOfDay? endTime;
  ...
  Future<void> _pickMusic() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => const MusicPickerScreen(multiPick: true),
      ),
    );

    if (result != null && result is List) {
      setState(() {
        selectedMusicList = List<Map<String, String>>.from(
          ...
          Padding(
            ...
            child: ListView(
              children: [
                ListTile(
                  title: Text(
                    selectedMusicList.isEmpty
                        ? "Pilih Musik"
                        : selectedMusicList.map((e) => e['title']).join(', '),
                  ),
                  trailing: const Icon(
                    Icons.library_music,
                ...
                OutlinedButton(
                  ...
                  style: OutlinedButton.styleFrom(
                    foregroundColor: const Color.fromARGB(255, 81, 177, 255),
